...
int8_t STORAGE_Read (uint8_t lun,uint8_t *buf,uint32_t blk_addr,uint16_t blk_len)
{
	...
	{
        //ReadBytesOffset(blk_addr*4096,buf,0, blk_len*4096);
        ReadBytes(blk_addr,buf,0, blk_len*FLASH_SECTOR_SIZE);
...
int8_t STORAGE_Write (uint8_t lun,uint8_t *buf,uint32_t blk_addr,uint16_t blk_len) 
{
    ...
    else
	{
        ...
        WriteBytes(blk_addr,buf,blk_len*FLASH_SECTOR_SIZE);
	} 
	return res; 
}
//获取支持的最大逻辑单元个数
